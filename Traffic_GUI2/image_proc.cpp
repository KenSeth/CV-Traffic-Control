...
{
//    QMutexLocker locker(&m_mutex);
    static std::vector<cv::Rect> *rectFW, *rectTR, *rectMT;
                ...
                if(cfg.classifierFW.empty() == false)
                {
                    ...
                    if(toggleDebug == true){
                        for(int j = 0; j < rectFW->size() ;j++)
                        {
                            ...
                            (*rectFW)[j].width = (*rectFW)[j].width;

                            //groupRectangles(*rectFW,1,0.2);
                ...
                if(cfg.classifierMT.empty() == false)
                {
                    ...
                    if(toggleDebug == true){
                        for(int j = 0; j < rectMT->size() ;j++)
                        {
                            ...
                            cv::rectangle(matFrameDraw[i],(*rectMT)[j],cv::Scalar(0,255,0),8);
